    ...
    const steps = {
        welcome: {
            ...
            content: () => createElement(
                ...
                createElement('h2', null, 'Choose your project type'),
                createElement(
                    'div',
            ...
            content: () => {
                const getPreviewContent = () => {
                    switch(setupData.setup_type) {
                        case 'nextjs_forms':
                            return {
                                description: 'Perfect for tracking form submissions from your NextJS/React applications',
                                contentTypes: ['Contact Form', 'Newsletter Signup', 'Support Ticket'],
...
const submitForm = async (data) => {
  await fetch('/wp-json/atlaspress/v1/content-types/1/entries', {
    ...
    body: JSON.stringify({ title: 'Contact Form', data })
                        ...
                        case 'headless_cms':
                            return {
                                description: 'Manage your content with a powerful headless CMS',
                                contentTypes: ['Blog Post', 'Product', 'Page Content'],
...
const posts = await fetch('/wp-json/atlaspress/v1/content-types/1/entries')
  .then(r => r.json());`
                        ...
                        case 'api_backend':
                            return {
                                description: 'Store and manage structured data via REST/GraphQL APIs',
                                contentTypes: ['User Profile', 'API Log', 'Settings'],
...
const DashboardApp = () => {
    ...
    return createElement(
        ...
        createElement(
            ...
            createElement(StatCard, { label: 'Content Types', value: stats.contentTypes, color: '#5F7A61' }),
            createElement(StatCard, { label: 'Fields', value: stats.fields, color: '#9CA3AF' }),
            createElement(StatCard, { label: 'Entries', value: stats.entries, color: '#5F7A61' }),
        ...
        selectedType ? createElement(
            ...
            entries.length === 0
                ? createElement(
                    'div',
                    { className: 'postbox', style: { padding: '40px', textAlign: 'center' } },
                    createElement('h3', { style: { color: '#666' } }, 'No submissions yet'),
                    createElement('p', null, 'Form submissions will appear here when users submit forms from your frontend.'),
                    createElement(
                        'div',
                        { style: { marginTop: '20px', padding: '15px', background: '#f8f9fa', borderRadius: '6px', textAlign: 'left' } },
                        createElement('h4', { style: { margin: '0 0 10px 0' } }, 'Integration Code:'),
                        createElement('pre', {
                            style: {
                                background: '#fff',
                                padding: '12px',
  ...
  body: JSON.stringify({
    ...
    data: { name: 'John', email: 'john@example.com' }
                ...
                : createElement(
                    ...
                    createElement(
                        'div',
                        { style: { padding: '15px', background: '#f0f6fc', borderRadius: '6px', marginBottom: '15px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' } },
                        createElement(
                            'div',
                            null,
                            createElement('strong', null, `${entries.length} submissions`),
                            createElement('span', { style: { marginLeft: '15px', color: '#666' } }, 
